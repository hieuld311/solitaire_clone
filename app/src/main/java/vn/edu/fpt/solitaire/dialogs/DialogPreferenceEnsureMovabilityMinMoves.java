...
import static vn.edu.fpt.solitaire.SharedData.lg;
import static vn.edu.fpt.solitaire.SharedData.showToast;
import static vn.edu.fpt.solitaire.helper.Preferences.PREF_KEY_ENSURE_MOVABILITY_MIN_MOVES;
...
public class DialogPreferenceEnsureMovabilityMinMoves
    ...
    public void onClick(View view) {
        if (view.getId() == R.id.settings_ensure_movability_make_games_winnable) {
            for (int i = 0; i < gameCount; i++) {
                if (gameInfoList.get(i).canStartWinnableGame()) {
                    inputs.get(i).setText(winnableText);
                }
            }
        } else if (view.getId() == R.id.settings_ensure_movability_reset) {
            for (int i = 0; i < gameCount; i++) {
